 ...
 * Now includes comprehensive automation orchestration with response extraction,
 * error handling, retry logic, and progress management
 */
...
class AutomationManager {
    constructor() {
        ...
        this.retryAttempts = new Map(); // Track retry attempts per prompt
        
        this.settings = {
            delay: 3000, // Default 3 second delay between prompts
            maxRetries: 3,
            timeout: 45000, // 45 second timeout per prompt (increased for response extraction)
            retryDelay: 5000, // 5 second delay before retry
            responseTimeout: 30000, // 30 seconds to wait for AI response
            enableRetries: true,
            pauseOnError: false
        };
    ...
    async resumeAutomation() {
        ...
        // Continue processing from current prompt
        await this.processNextPrompt();
    }
    ...
    async processNextPrompt() {
        ...
        try {
            ...
            // Send prompt to content script with enhanced message
            await browser.tabs.sendMessage(this.currentTabId, {
                type: 'execute-prompt',
                prompt: currentPrompt,
    ...
    async handlePromptCompleted(result) {
        ...
        this.retryAttempts.delete(this.currentPromptIndex - 1); // Clean up retry count
        await this.saveAutomationState();
    ...
    // ... (keeping all the existing helper methods from the original file)

    async handleContentScriptReady(tabId) {
        this.log(`Content script ready on tab ${tabId}`);
        if (this.isRunning && this.currentTabId === tabId && !this.isPaused) {
            await this.processNextPrompt();
        }
    }
